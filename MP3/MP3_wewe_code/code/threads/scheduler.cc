...
int sjf(Thread* First , Thread* Second){
    int retVal = 0;
    int rem_burst_time1 = (First -> getBurstTime()) - (First -> getAccumulatedTime());
    int rem_burst_time2 = (Second -> getBurstTime()) - (Second -> getAccumulatedTime());
...
Scheduler::ReadyToRun (Thread *thread)
{
    ...
    resumes running.*/
    thread->setLastTick(kernel->stats->totalTicks);
...
Scheduler::FindNextToRun()
{
    ...
    if(!L1_list->IsEmpty()){
        ...
        next->setLastTick(kernel->stats->totalTicks);
        /*modified end*/
...
Scheduler::Run (Thread *nextThread, bool finishing)
{
    ...
    "] is replaced, and it has executed [" <<kernel->stats->totalTicks - oldThread->getLastTick()<<"] ticks) \n");
    /*modified end*/    
    ...
    oldThread->setLastTick(kernel->stats->totalTicks);
    //the thread start waiting for CPU
    //nextThread->setLastTick(kernel->stats->totalTicks);
    /*modified end*/
