	...
	kernel->scheduler->Re_Sort_List();
        ...
        if(kernel->currentThread->getPriority() > 99){
            if(kernel->scheduler->L1_empty() == false){
                ...
                double rem_burst_time1 = (cur_t -> getBurstTime()) - (cur_t -> getAccumulatedTime());
                double rem_burst_time2 = (L1_front -> getBurstTime()) - (L1_front -> getAccumulatedTime());
